    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append((((nextx, nexty), visitedCorner), action, 1))
        self._expanded += 1  # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    while len(unseen) != 0:
        ...
        for corner in unseen:
            h = util.manhattanDistance(corner, curr)
...
def foodHeuristic(state, problem):
    ...
    for food in foods:
        parent[food] = 0
        priority[food] = 0
        foodh = mhd(position, food)
        if next_foodh is None or foodh < next_foodh:
            next_foodh = foodh
        all_possible_nodes = []
        for value in foods[1:]:
            node = (mhd(food, value), food, value)
            all_possible_nodes.append(node)
        mst.extend(all_possible_nodes)
    mst.sort(key=lambda x: x[0])
    i, j, msth = 0, 0, 0
    while j < len(foods) - 1:
        i += 1
        x, y, equal = x_y_equal(mst[i][1], mst[i][2])
        if not equal:
            msth += mst[i][0]
            childx, childy, _ = x_y_equal(x, y)
            # union
            rx = priority[childx]
            ry = priority[childy]
            if rx < ry:
                ...
                parent[childy] = childx
            elif rx == ry:
                parent[childy] = childx
                priority[childx] += 1
            j += 1
    resh = next_foodh + msth
    return resh
